...
public:
    ...
    static Buffer* createFromStdin() {
        ...
        DWORD _size;
        do {
            std::cout << "Please the type of memory to be accessed (0 = string, 1 = long, 2 = unsigned long, 3 = long long, 4 = unsigned long long, 5 = float, 6 = double): ";
            std::cin >> _type;
            if (_type == 0) {
...
public:
    ...
    void readMem() {
        ...
        printf("Memory at address 0x%p in process PID %lu is '0x", (PVOID)addressMem, pid);
        buf->output(true);
        printf("' (hex) '");
        buf->output(false);
